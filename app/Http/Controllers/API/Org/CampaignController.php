...
{
    public function __construct(private CampaignService $service, private readonly NotificationEventService $notifications) {}

    public function index(): AnonymousResourceCollection
    {
        $this->authorize('viewAnyOrganization', Campaign::class);
        return CampaignResource::collection($this->service->paginate(request()->all(), $this->organizationId()));
    }
    ...
    public function update(CampaignRequest $request, Campaign $campaign): CampaignResource
    {
        ...
        if ($hasCampaignUpdateFields || ! array_key_exists('status', $validated)) $this->authorize('updateOrganization', $campaign);
        if ($hasCampaignUpdateFields) {
            $campaign = $this->service->update($campaign, CampaignData::from($this->campaignData($campaign, $validated)))->refresh();
        }
        if (array_key_exists('status', $validated)) {
            ...
            if ($status === 'closed' && ! $wasClosed) $this->notifyCampaignClosed($campaign);
        }
        return CampaignResource::make($campaign->refresh()->load(['imageMedia', 'category']));
    }
    ...
    private function notifyCampaignClosed(Campaign $campaign): void
    {
        ...
        $this->notifications->notifyOrganization((string) $campaign->organization_id, NotificationEventType::CampaignClosed, 'تم إغلاق الحملة', $body, 'campaign', 'normal', $campaign->title, '/org/campaigns/'.$campaign->id, $creatorId);
        $this->notifications->notifyCampaignParticipants($campaign, NotificationEventType::CampaignClosed, 'انتهت الحملة', $body, 'campaign', 'normal', $campaign->title, '/campaigns/'.$campaign->id, $creatorId);
    }
    ...
    private function hasCampaignUpdateFields(array $validated): bool
    {
        return count(array_intersect(array_keys($validated), [
            'title', 'summary', 'categoryId', 'audience', 'location', 'goalAmount', 'beneficiariesCount', 'startDate', 'endDate',
        ])) > 0;
    }
    ...
    private function campaignData(Campaign $campaign, array $validated): array
    {
        return [
            ...
            'audience' => $validated['audience'] ?? $campaign->audience ?? 'general',
