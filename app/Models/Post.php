...
#[Fillable([
    'id', 'title', 'summary', 'content', 'type', 'audience', 'status', 'help_status', 'urgency', 'urgency_reason',
    'location', 'organization_id', 'campaign_id', 'category_id', 'author_id', 'updated_by',
...
class Post extends Model
{
    ...
    protected static function booted(): void
    {
        static::saving(function (Post $post): void {
            if ($post->type === 'help_request' && $post->help_status === null) $post->help_status = HelpRequestStatus::Open;
    ...
    {
        return $this->helpOffers()->whereIn('status', array_map(static fn (HelpOfferStatus $status) => $status->value, [HelpOfferStatus::Pending, HelpOfferStatus::Accepted, HelpOfferStatus::Contacting, HelpOfferStatus::Agreed]));
    }

    public function completedHelpOffers(): HasMany { return $this->helpOffers()->where('status', HelpOfferStatus::Completed->value); }
    public function campaignApplications(): HasMany { return $this->hasMany(CampaignApplication::class, 'campaign_id', 'campaign_id')->whereNotIn('applicant_status', ['rejected', 'withdrawn']); }
