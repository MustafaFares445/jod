...
class PersonalizedFeedService
{
    public function paginate(User $viewer, FeedType $type, int $page = 1, int $perPage = 20): LengthAwarePaginator
    {
        ...
        if ($type === FeedType::Nearby) $query->where('location', $location);
        if ($type === FeedType::Urgent) $query->whereIn('urgency', [PostUrgency::Important->value, PostUrgency::Urgent->value, PostUrgency::Critical->value]);
        $candidates = $query->orderByDesc('published_at')->orderByDesc('created_at')->limit((int) config('recommendations.candidate_limit', 200))->get();
        return $this->paginator($this->rank($viewer, $candidates), $page, $perPage);
    }
    ...
    private function rank(User $viewer, Collection $candidates): Collection
    {
        ...
        return $candidates->reject(fn (Post $post): bool => $excludedPostIds->has((string) $post->id) || $hiddenPublishers->has($this->publisherKey($post)))
            ->map(function (Post $post) use ($viewer, $preference, $interests, $followedUsers, $followedOrganizations, $viewCounts): array {
                $scored = $this->score($viewer, $post, $preference?->intent, $preference?->preferred_city ?? $viewer->city, $interests->get($post->category_id), $followedUsers->has((string) $post->author_id), $post->organization_id !== null && $followedOrganizations->has((string) $post->organization_id), (int) ($viewCounts[$post->id] ?? 0));
    ...
    private function score(User $viewer, Post $post, ?UserIntent $intent, ?string $preferredCity, ?UserCategoryInterest $interest, bool $followsAuthor, bool $followsOrganization, int $viewCount): array
    {
        ...
        if ($this->intentMatches($intent, $post)) $components['intent_match'] = (float) $weights['intent_match'];
        if ($viewer->relationLoaded('capabilities') || $viewer->capabilities()->exists()) {
            $requiredIds = $post->requiredCapabilities->pluck('id');
            if ($requiredIds->isNotEmpty() && $viewer->capabilities()->whereIn('capabilities.id', $requiredIds)->exists()) $components['capability_match'] = (float) $weights['capability_match'];
        }
        $freshness = $this->freshnessScore($post); if ($freshness > 0) $components['fresh'] = $freshness;
    ...
    {
        $publishedAt = $post->published_at ?? $post->created_at; if ($publishedAt === null) return 0;
