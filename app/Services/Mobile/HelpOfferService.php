...
class HelpOfferService
{
    ...
    /** @param array{type:string, amount?:int|float|string|null, description?:string|null, contactMethod?:string|null, phone?:string|null} $attributes */
    public function create(User $helper, string $postId, array $attributes): HelpOffer
    {
        $offer = DB::transaction(function () use ($helper, $postId, $attributes): HelpOffer {
            ...
            if ($post->help_status === HelpRequestStatus::Fulfilled) {
                throw ValidationException::withMessages([
                    'post' => ['This help request has already been fulfilled.'],
                ]);
            }

            if ($post->author_id === null) {
                throw ValidationException::withMessages([
                    'post' => ['This help request does not have an owner who can receive offers.'],
                ]);
            }

            if ((string) $post->author_id === (string) $helper->id) {
                throw ValidationException::withMessages([
                    'post' => ['You cannot offer help on your own help request.'],
                ]);
            }

            Gate::forUser($helper)->authorize('create', [HelpOffer::class, $post]);

            $duplicate = HelpOffer::query()
            ...
            if ($duplicate) {
                throw ValidationException::withMessages([
                    'post' => ['You already have an active help offer for this request.'],
                ]);
            }
            ...
            return HelpOffer::query()->create([
                ...
                'contact_method' => $attributes['contactMethod'] ?? null,
                'phone' => $attributes['phone'] ?? $helper->phone,
            ]);
        });
    ...
    /** @param array{perPage?:int,status?:string,postId?:string,flow?:string} $params */
    public function paginateForUser(User $user, array $params): LengthAwarePaginator
    {
        ...
        return HelpOffer::query()
            ...
            ->when(
                $flow === 'received',
                fn (Builder $query) => $query->where('post_owner_id', $user->id),
                fn (Builder $query) => $query->where('helper_user_id', $user->id),
            )
    ...
    public function findForUser(User $user, string $offerId): ?HelpOffer
    {
        ...
        if ($offer === null || ! Gate::forUser($user)->allows('view', $offer)) {
            return null;
        }
    ...
    {
        $offer = $this->transition($actor, $offerId, 'accept', [HelpOfferStatus::Pending], HelpOfferStatus::Accepted, [
            'accepted_at' => now(),
        ]);
    ...
    {
        $offer = $this->transition($actor, $offerId, 'reject', [HelpOfferStatus::Pending], HelpOfferStatus::Rejected, [
            'rejected_at' => now(),
    ...
    {
        $offer = $this->transition($actor, $offerId, 'coordinate', [HelpOfferStatus::Accepted], HelpOfferStatus::Contacting, [
            'contacted_at' => now(),
        ]);
        $this->helpStatus->sync($offer->post);
    ...
    {
        $offer = $this->transition($actor, $offerId, 'coordinate', [HelpOfferStatus::Contacting], HelpOfferStatus::Agreed, [
            'agreed_at' => now(),
        ]);
        $this->helpStatus->sync($offer->post);
    ...
    public function cancel(User $actor, string $offerId, string $reason): HelpOffer
    {
        $offer = DB::transaction(function () use ($actor, $offerId, $reason): HelpOffer {
            ...
            if (! in_array($offer->status, [
                ...
                HelpOfferStatus::Accepted,
                HelpOfferStatus::Contacting,
                HelpOfferStatus::Agreed,
            ], true)) {
                $this->throwInvalidTransition($offer, HelpOfferStatus::Cancelled);
            }

            $offer->forceFill([
                'status' => HelpOfferStatus::Cancelled,
                'cancelled_at' => now(),
    ...
    public function confirmProvided(User $helper, string $offerId): HelpOffer
    {
        return $this->confirm($helper, $offerId, 'confirmProvided', 'helper_confirmed_at', NotificationEventType::HelpOfferHelperConfirmed);
    }

    public function confirmReceived(User $owner, string $offerId): HelpOffer
    {
        return $this->confirm($owner, $offerId, 'confirmReceived', 'receiver_confirmed_at', NotificationEventType::HelpOfferReceiverConfirmed);
    }
    ...
    public function updateHelpStatus(User $owner, string $postId, HelpRequestStatus $status): Post
    {
        ...
        if ($status === HelpRequestStatus::Fulfilled) {
            ...
            $this->notifications->notifyUser(
                $owner,
                NotificationEventType::HelpRequestFulfilled,
                'تم إغلاق طلب المساعدة',
                'تم تسجيل أن الحاجة تمت تلبيتها بالكامل ولن يستقبل الطلب عروضاً جديدة.',
                'help',
                'normal',
                $post->title,
                '/posts/'.$post->id,
        ...
        if ($status === HelpRequestStatus::Open) {
            ...
            $this->notifications->notifyUser(
                $owner,
                NotificationEventType::HelpRequestReopened,
                'تم فتح طلب المساعدة',
                'الطلب متاح مجدداً لاستقبال عروض المساعدة.',
                'help',
                'normal',
                $post->title,
                '/posts/'.$post->id,
        ...
        throw ValidationException::withMessages([
            'status' => ['The owner may explicitly set a help request to fulfilled or open. in_progress is derived from active accepted offers.'],
        ]);
    }

    /** @param list<HelpOfferStatus> $from @param array<string,mixed> $values */
    private function transition(User $actor, string $offerId, string $ability, array $from, HelpOfferStatus $to, array $values): HelpOffer
    {
        return DB::transaction(function () use ($actor, $offerId, $ability, $from, $to, $values): HelpOffer {
            ...
            if (! in_array($offer->status, $from, true)) {
                $this->throwInvalidTransition($offer, $to);
            }
    ...
    private function confirm(User $actor, string $offerId, string $ability, string $column, NotificationEventType $event): HelpOffer
    {
        ...
        $offer = DB::transaction(function () use ($actor, $offerId, $ability, $column, &$completedNow): HelpOffer {
            ...
            if ($offer->status === HelpOfferStatus::Completed) {
                return $offer;
            }

            if ($offer->status !== HelpOfferStatus::Agreed) {
                $this->throwInvalidTransition($offer, HelpOfferStatus::Completed);
            }

            if ($offer->{$column} === null) {
                $offer->forceFill([$column => now()])->save();
            }
    ...
    {
        throw ValidationException::withMessages([
            'status' => ["Help offer cannot transition from {$offer->status->value} to {$to->value}."],
        ]);
    }
    ...
    {
        if ((string) $actor->id === (string) $offer->helper_user_id) {
