...
{
    public function __construct(private readonly PersonalizedFeedService $feed, private readonly AdminUserPersonalizationService $personalization) {}
    public function preview(User $user, int $limit = 20): array
    {
        ...
        return [
            ...
            'recommendations' => collect($paginator->items())->map(function (array $item): array {
                $model = $item['model'];
                return ['contentType' => $item['contentType'], 'contentId' => (string) $model->getKey(), 'title' => (string) ($model->title ?? $model->description ?? ''), 'score' => (float) ($item['score'] ?? 0), 'reasons' => $item['reasons'] ?? [], 'components' => $item['components'] ?? []];
            })->values()->all(),
        ];
    }
}
