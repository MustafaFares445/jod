...
class AdminHelpMatchingService
{
    ...
    public function detail(Post $post): array
    {
        ...
        $summary['matchingCriteria'] = [
            'categoryId' => $post->category_id,
            'city' => $post->location,
            'intent' => [UserIntent::Giver->value, UserIntent::Both->value],
        ];
        $summary['offers'] = $post->helpOffers()->latest()->limit(50)->get()->map(fn ($offer) => [
            'id' => (string) $offer->id,
            'status' => $offer->status?->value ?? $offer->status,
            'userId' => (string) $offer->helper_user_id,
            'createdAt' => $offer->created_at?->toIso8601String(),
        ])->all();
        return $summary;
    }
    ...
    {
        $potentialHelpers = User::query()
            ->whereHas('preference', function (Builder $q) use ($post): void {
            ...
            ->when($post->category_id, fn (Builder $q) => $q->whereHas('categoryInterests', fn (Builder $interest) => $interest
                ->where('category_id', $post->category_id)
                ->where(fn (Builder $w) => $w->where('explicit_weight', '>', 0)->orWhere('behavioral_weight', '>', 0))))
        ...
        return [
            'request' => [
                ...
                'city' => $post->location,
                'urgency' => $post->urgency?->value ?? $post->urgency,
                'status' => $post->help_status?->value ?? $post->help_status,
