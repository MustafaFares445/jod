...
final class SyrianRealDemoSeeder extends Seeder
{
    ...
    {
        $data = json_decode((string) file_get_contents(database_path('data/syrian_real_seed.json')), true, 512, JSON_THROW_ON_ERROR);
    ...
    {
        foreach (['post_feedback','recommendation_impressions','user_interactions','post_capabilities','post_likes','saved_posts','help_offers','campaign_likes','campaign_applications','donations','media','posts','campaigns','organization_staff','organization_roles'] as $table) {
            if (Schema::hasTable($table)) DB::table($table)->delete();
        }
        if (Schema::hasTable('publisher_follows')) DB::table('publisher_follows')->where('target_type', 'organization')->delete();
        if (Schema::hasTable('hidden_publishers')) DB::table('hidden_publishers')->where('target_type', 'organization')->delete();
        if (Schema::hasTable('organizations')) DB::table('organizations')->delete();
        if (Schema::hasTable('users')) DB::table('users')->whereNotNull('organization_id')->update(['organization_id' => null, 'updated_at' => now()]);
    }
    ...
    private function seedCategories(array $rows): void
    {
        foreach ($rows as [$key, $name, $description]) {
            ...
            $this->upsert('categories', ['id' => $id], [
                'id' => $id, 'name' => $name, 'description' => $description, 'status' => 'active',
                'usage_count' => 0, 'deleted_at' => null, 'created_at' => now(), 'updated_at' => now(),
            ]);
        }
    }
    ...
    private function seedOrganizations(array $rows): void
    {
        foreach ($rows as $org) {
            ...
            $demoCode = strtoupper(substr(hash('sha256', $key), 0, 12));

            $this->upsert('organizations', ['id' => $id], [
                'id' => $id,
                'name' => $org['name'],
                'email' => $org['email'] ?? strtolower($demoCode).'@demo.jod.local',
                'phone' => $org['phone'] ?? null,
                'organization_number' => 'JOD-DEMO-'.$demoCode,
                'organization_type' => $org['type'],
                'registration_number' => $org['registration_number'] ?? null,
                'bank_account_number' => 'JOD-DEMO-ACCOUNT-'.$demoCode,
                ...
                'iban' => 'JOD-DEMO-'.$demoCode,
                'status' => 'active', 'verification_status' => 'verified', 'accepted_at' => now()->subMonths(8),
                'campaigns_count' => 0, 'posts_count' => 0, 'active_volunteers_count' => 1,
                'activity_score' => 8.5, 'last_active_at' => now()->subHours(2), 'deleted_at' => null,
                'created_at' => now()->subMonths(8), 'updated_at' => now(),
            ]);

            $this->upsert('users', ['id' => $ownerId], [
                'id' => $ownerId,
                'name' => 'حساب إدارة '.$org['name'],
                'email' => 'owner+'.strtolower($demoCode).'@demo.jod.local',
                'user_type' => 'general', 'organization_id' => $id, 'status' => 'active',
                'email_verified_at' => now(), 'password' => Hash::make('demo-password'),
                'last_active_at' => now()->subHours(3), 'created_at' => now()->subMonths(8), 'updated_at' => now(),
            ]);

            $roleId = $this->id('role:'.$key.':owner');
            $this->upsert('organization_roles', ['id' => $roleId], [
                'id' => $roleId, 'organization_id' => $id, 'name' => 'Owner', 'description' => 'JOD demo owner role',
                'permissions' => json_encode([], JSON_THROW_ON_ERROR), 'is_active' => true, 'is_system' => true,
                'members_count' => 1, 'created_at' => now(), 'updated_at' => now(),
            ]);
            $this->upsert('organization_staff', ['organization_id' => $id, 'user_id' => $ownerId], [
                'organization_id' => $id, 'user_id' => $ownerId, 'organization_role_id' => $roleId,
                'name' => 'حساب إدارة '.$org['name'], 'email' => 'owner+'.strtolower($demoCode).'@demo.jod.local',
                'status' => 'active', 'invited_at' => now(), 'accepted_at' => now(), 'created_at' => now(), 'updated_at' => now(),
            ]);
    ...
    private function seedCampaigns(array $organizations): void
    {
        foreach ($organizations as $org) {
            ...
            foreach ($org['campaigns'] as $index => $row) {
                ...
                $details = $provenance === 'documented'
                    ? 'اسم الحملة أو المشروع موثق في المصدر المرجعي.'
                    : ($provenance === 'documented_preparation' ? 'المصدر يوثق التحضير أو الإطلاق ولا يثبت حصيلة نهائية.' : 'سجل JOD مبني على برنامج أو قطاع عمل موثق للجهة.');
                if ($reportedPledge !== null) $details .= "\nالقيمة المنشورة للتبرعات/التعهدات: ".number_format((float) $reportedPledge, 0).' دولار، ولا تعامل تلقائياً كمبلغ محصل داخل JOD.';

                $this->upsert('campaigns', ['id' => $campaignId], [
                    'id' => $campaignId, 'title' => $title,
                    ...
                    'status' => $status, 'location' => $location, 'images' => json_encode([], JSON_THROW_ON_ERROR),
                    'organization_id' => $this->organizations[$orgKey], 'creator_id' => $this->owners[$orgKey],
                    'goal_amount' => $goal ?? 0, 'raised_amount' => $raised ?? 0,
                    'beneficiaries_count' => 0, 'donors_count' => 0, 'applicants_count' => 0,
                    ...
                    'deleted_at' => null, 'created_at' => now()->subDays(90 + $index), 'updated_at' => now()->subDays($index % 8),
                ]);
                ...
                if ($status === 'closed') $this->seedCampaignPost($orgKey, $campaignId, $title, $category, $location, 'campaign_summary', $source, $image, ($index * 3) + 2);
            }
        }
    }
    ...
    private function seedCampaignPost(string $orgKey, string $campaignId, string $campaignTitle, string $category, string $location, string $type, string $source, ?string $image, int $offset): void
    {
        ...
        $this->upsert('posts', ['id' => $postId], [
            'id' => $postId, 'title' => $prefix.' '.$campaignTitle,
            ...
            'type' => $type, 'audience' => in_array($category, ['education','employment'], true) && $offset % 7 === 0 ? 'student' : 'general',
            'status' => 'published', 'location' => $location, 'organization_id' => $this->organizations[$orgKey],
            'campaign_id' => $campaignId, 'category_id' => $this->categories[$category], 'author_id' => $this->owners[$orgKey],
            'views_count' => 120 + (($offset * 37) % 4200), 'reactions_count' => 8 + (($offset * 11) % 340), 'applications_count' => 0,
            'published_at' => now()->subDays(30 + ($offset % 100)), 'deleted_at' => null,
            'created_at' => now()->subDays(35 + ($offset % 100)), 'updated_at' => now()->subDays($offset % 8),
        ]);
    ...
    private function seedSpecialPosts(array $rows): void
    {
        foreach ($rows as $index => $row) {
            ...
            $this->upsert('posts', ['id' => $postId], [
                'id' => $postId, 'title' => $title, 'summary' => $summary,
                'content' => $summary.' نوع التوثيق: '.$provenance.'.', 'type' => $type,
                'audience' => in_array($category, ['education','employment'], true) ? 'student' : 'general',
                'status' => 'published', 'location' => $location, 'organization_id' => $this->organizations[$orgKey],
                'campaign_id' => $campaignId, 'category_id' => $this->categories[$category], 'author_id' => $this->owners[$orgKey],
                'views_count' => 450 + ($index * 83), 'reactions_count' => 25 + ($index * 13),
                'applications_count' => in_array($type, ['job_opportunity','volunteer_opportunity'], true) ? 12 + $index : 0,
                'published_at' => now()->subDays(5 + $index), 'deleted_at' => null, 'created_at' => now()->subDays(6 + $index), 'updated_at' => now(),
            ]);
    ...
    private function seedHelpRequests(array $rows): void
    {
        ...
        foreach ($rows as $index => [$category,$location,$title,$summary]) {
            ...
            $this->upsert('posts', ['id' => $postId], [
                ...
                'type' => 'help_request', 'audience' => $category === 'education' ? 'student' : 'general', 'status' => 'published',
                'help_status' => $helpStatus, 'urgency' => $urgency,
                'urgency_reason' => $urgency === 'critical' ? 'حالة تجريبية حرجة لاختبار الترتيب والمراجعة.' : null,
                'location' => $location, 'organization_id' => null, 'campaign_id' => null,
                'category_id' => $this->categories[$category], 'author_id' => $users[$index % $users->count()],
                'views_count' => 70 + ($index * 31), 'reactions_count' => 3 + ($index * 4), 'applications_count' => 1 + ($index % 5),
                'published_at' => now()->subDays(2 + $index), 'expires_at' => $helpStatus === 'expired' ? now()->subDay() : now()->addDays(10 + $index),
                'fulfilled_at' => in_array($helpStatus, ['fulfilled','partially_fulfilled'], true) ? now()->subDay() : null,
                'deleted_at' => null, 'created_at' => now()->subDays(3 + $index), 'updated_at' => now(),
            ]);
    ...
    private function refreshCounters(): void
    {
        foreach ($this->categories as $id) {
            DB::table('categories')->where('id', $id)->update(['usage_count' => DB::table('campaigns')->where('category_id', $id)->count() + DB::table('posts')->where('category_id', $id)->count()]);
        }
    ...
    private function seedMedia(): void
    {
        foreach ($this->media as $row) {
            ...
            $path = $base.'.jpg'; $mime = 'image/jpeg'; $official = false;
            if ($source) {
                try {
                    $response = Http::timeout(15)->retry(1, 200)->get($source);
                    ...
                    if ($response->successful() && str_starts_with($contentType, 'image/')) {
                        ...
                        $path = $base.'.'.$ext; $mime = explode(';', $contentType)[0];
                        $disk->put($path, $response->body()); $official = true;
                    }
                } catch (Throwable $e) {
            ...
            if (! $official) {
                ...
                $path = 'demo/syria/fallback/'.$fallback.'.svg'; $mime = 'image/svg+xml';
                if (! $disk->exists($path)) $disk->put($path, $this->fallbackSvg($fallback));
            }
            $mediaId = $this->id('media:'.$row['type'].':'.$row['key']);
            $this->upsert('media', ['id' => $mediaId], [
                'id' => $mediaId, 'model_type' => $row['type'], 'model_id' => $row['id'],
                'post_id' => $row['type'] === 'post' ? $row['id'] : null,
                'prop' => $row['type'] === 'organization' ? 'logo' : 'images', 'disk' => 'public', 'path' => $path,
                'original_name' => basename($path), 'description' => $official ? 'صورة من المصدر الرسمي.' : 'صورة افتراضية ثابتة مرتبطة بطبيعة المحتوى.',
                'mime_type' => $mime, 'size' => $disk->exists($path) ? $disk->size($path) : 0,
    ...
    {
        $labels = ['health'=>'مشروع صحي','education'=>'مشروع تعليمي','food'=>'مشروع للأمن الغذائي','emergency'=>'استجابة طارئة','shelter'=>'مشروع إيواء','employment'=>'مشروع للتدريب وسبل العيش','volunteering'=>'مبادرة تطوعية','protection'=>'مشروع حماية','wash'=>'مشروع مياه وإصحاح','children'=>'مشروع لرعاية الأطفال','disability'=>'مشروع لدعم ذوي الإعاقة','reconstruction'=>'مشروع تعافٍ وإعادة إعمار'];
    ...
    {
        $labels = ['health'=>'الصحة','education'=>'التعليم','food'=>'الغذاء','emergency'=>'الطوارئ','shelter'=>'الإيواء','employment'=>'العمل والتدريب','wash'=>'المياه والإصحاح','reconstruction'=>'إعادة الإعمار','children'=>'الأطفال','disability'=>'ذوي الإعاقة','protection'=>'الحماية','volunteering'=>'التطوع','organization'=>'منظمة سورية'];
    ...
    private function upsert(string $table, array $where, array $attributes): void
    {
        ...
        if ($where !== []) DB::table($table)->updateOrInsert($where, $attributes);
    }
